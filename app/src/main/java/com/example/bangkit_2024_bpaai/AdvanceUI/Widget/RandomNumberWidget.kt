...
class RandomNumberWidget : AppWidgetProvider() {
    // onUpdate() adalah metode yang akan dipanggil ketika widget pertama kali dibuat
    ...
    // Apa yang di-broadcast tentunya harus ada receiver-nya
...
// metode yang dipanggil di setiap perulangan appWidgetIds, di mana hampir seluruh proses update ada di dalam metode ini
internal fun updateAppWidget(
    ...
    val views = RemoteViews(context.packageName, R.layout.random_number_widget)
    // di mana kita mengubah string text
    views.setTextViewText(R.id.appwidget_text, lastUpdate)
    views.setOnClickPendingIntent(R.id.btn_click, pendingIntent)

    // Instruct the widget manager to update the widget
    appWidgetManager.updateAppWidget(appWidgetId, views)
    // parameter 1 adalah id widget yang ingin kita update, sedangkan parameter 2 adalah RemoteViews yang berisikan views yang telah kita modifikasi
}